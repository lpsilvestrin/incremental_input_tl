...
def plot_mean_input_comparison():
    """
    compare the MSE between data-pooling and regular mean inputation in the simulated data
    :return:
    """
    theta = np.array([2., -2])
    noise_var = 1
    ...
    def compute_params(Xs, Ys, Xt, Yt):
        ...
        # compute mean inputation
        xs_mi = np.concatenate([Xs, mu[1] * np.ones([len(Xs), 1])], axis=1)
        x_mi = adi(np.concatenate([xs_mi, Xt]))
        y_mi = np.concatenate([Ys, Yt])

        w_mi = SolveRidgeRegression(x_mi, y_mi, 0, lam=.0)

        # compute basic OLS estimator
        ixt = adi(Xt)
        wt = SolveRidgeRegression(ixt, Yt, 0, lam=.0)

        return wt, wWLS, w_mi, mu
    ...
    for _ in range(200):
        # Xs, Ys, Xt, Yt = sim_data(Ns, Nt, theta)
        tmp.extend([[compute_params(*sim_data(Ns, n, theta)), n] for n in range(5, Nt+1)])
    params.extend([[n, "dp", mse2(p[1], p[3])] for [p, n] in tmp])
    params.extend([[n, "ols", mse(p[0])] for [p, n] in tmp])
    params.extend([[n, "mi", mse(p[2])] for [p, n] in tmp])
    # params.extend([[n, f"fine-tuning k = {k}", *p[2], mse(p[2])] for [p, n] in tmp])
    y_axis_name = "MSE"
    params = pd.DataFrame(params, columns=["$n_T$", "approach", y_axis_name])
    plt.figure(figsize=(6,4))
